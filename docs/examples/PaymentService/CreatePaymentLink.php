<?php
  ...
  use Payer\Api\PaymentService\CreatePaymentLinkRequest;
  ...
  try {
    ...
    $createPaymentLinkRequest = new CreatePaymentLinkRequest($env['wsdl_location']['payment_service'], $env['options']);
    $createPaymentLinkResponse = $createPaymentLinkRequest->create([
      'session' => $createSessionResponse,
      'request' => [
        'orderId' => $createOrderResponse->orderId,
        'paymentMethod' => 'CARD',
        'backToShopURL' => 'http://localhost',
        'redirectOnSuccessURL' => 'http://localhost',

      ]
    ]);

    var_dump($createPaymentLinkResponse);
